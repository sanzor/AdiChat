...
validate_user_data(_UserData=#create_user_params{name = Name}) when not is_binary(Name)-> {false,[{invalid_name,Name}]};
validate_user_data(_UserData)->true.


validate_topic_data(_TopicData=#create_topic_params{name = Name}) when not is_binary(Name)-> {false,[{invalid_name,Name}]};
